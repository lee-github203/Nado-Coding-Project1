...
// 사용자로부터 2개의 입력밧을 받아서 -> 같은 동물 찾으면 카드 뒤집기
// 모든 동물 쌍을 찾으면 게임 종료
// 총 실패 횟수 알려주기
...
int main_m_array_project(void)
{
	...
	while (1)
	{
		...
		if ((checkAnimal[firstSelect_x][firstSelect_y] == 0
			&& checkAnimal[secondSelect_x][secondSelect_y] == 0) // 카드가 뒤집히지 않았을 때
...
int getEmptyposition() // 좌표에서 빈 공간 찾기
{
	...
	while (1)
	{
		int randPos = rand() % 20; // 0 ~ 19 사이의 숫자 변환
		// 19 -> (3, 4)
...
void printQuestion() // 문제 출력 (카드 지도)
{
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 5; j++)
		{
			...
			// 아직 뒤집지 못했으면 (정답 못 맞췄으면) 뒷면 -> 위치를 나타내는 숫자
			else
			{
				printf("%8d", seq);
			}
